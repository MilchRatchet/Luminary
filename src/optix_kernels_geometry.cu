...
extern "C" __global__ void __raygen__optix() {
  ...
  for (int i = 0; i < task_count; i++) {
    ...
    if (bounce_info.is_transparent_pass) {
      ...
      is_delta_distribution |= (data.ior_in == data.ior_out) || (data.roughness < 0.02f);
    }
    else {
      is_delta_distribution |= bounce_info.is_microfacet_based && (data.roughness < 0.02f);
    }

    uint32_t state_flags_to_release = STATE_FLAG_BOUNCE_LIGHTING;
    if (!is_delta_distribution) {
      state_flags_to_release |= STATE_FLAG_DELTA_PATH;
    }
    ...
    if (validate_trace_task(bounce_task, bounce_record)) {
      ...
      state_release(pixel, state_flags_to_release);
    }
  }
