...
static LuminaryResult _lum_builtin_camera_physical_serialize(const LuminaryCameraPhysical* camera, LumBuiltinCameraPhysical* dst_camera) {
  __CHECK_NULL_ARGUMENT(camera);
  __CHECK_NULL_ARGUMENT(dst_camera);
...
static LuminaryResult _lum_builtin_cloud_layer_serialize(const LuminaryCloudLayer* layer, LumBuiltinCloudLayer* dst_layer) {
  __CHECK_NULL_ARGUMENT(layer);
  __CHECK_NULL_ARGUMENT(dst_layer);
...
LuminaryResult lum_builtin_cloud_serialize(const LuminaryCloud* cloud, LumBuiltinCloud* dst_cloud) {
  ...
  __FAILURE_HANDLE(_lum_builtin_cloud_layer_serialize(&cloud->top, &dst_cloud->top));
...
LuminaryResult lum_builtin_material_serialize(const LuminaryMaterial* material, LumBuiltinMaterial* dst_material) {
  ...
  dst_material->normal_map_is_compressed = material->normal_map_is_compressed;
  dst_material->bidirectional_emission   = material->bidirectional_emission;
  dst_material->albedo_tex.id            = material->albedo_tex;
  dst_material->luminance_tex.id         = material->luminance_tex;
  dst_material->roughness_tex.id         = material->roughness_tex;
  dst_material->metallic_tex.id          = material->metallic_tex;
  dst_material->normal_tex.id            = material->normal_tex;
...
LuminaryResult lum_builtin_instance_serialize(const LuminaryInstance* instance, LumBuiltinInstance* dst_instance) {
  ...
  dst_instance->mesh.id  = instance->mesh_id;
  dst_instance->position = instance->position;
  dst_instance->rotation = instance->rotation;
  dst_instance->scale    = instance->scale;
