...
local int inflateStateCheck(strm) z_streamp strm;
{
...
void makefixed() {
  ...
  for (;;) {
    if ((low % 7) == 0)
      printf("\n        ");
    printf(
      "{%u,%u,%d}", (low & 127) == 99 ? 64 : state.lencode[low].op, state.lencode[low].bits,
...
int flush;
{
  ...
  for (;;)
    ...
    case NAME:
      if (state->flags & 0x0800) {
        ...
        do {
          ...
          if (
            state->head != Z_NULL && state->head->name != Z_NULL
    ...
    case COMMENT:
      if (state->flags & 0x1000) {
        ...
        do {
          len = (unsigned) (next[copy++]);
          if (
            state->head != Z_NULL && state->head->comment != Z_NULL
    ...
    case CODELENS:
      ...
      state->lenbits = 9;
      ret            = inflate_table(
        LENS, state->lens, state->nlen, &(state->next), &(state->lenbits), state->work);
      if (ret) {
      ...
      state->distbits = 6;
      ret             = inflate_table(
        DISTS, state->lens + state->nlen, state->ndist, &(state->next), &(state->distbits),
        state->work);
      if (ret) {
...
int ZEXPORT inflateValidate(strm, check) z_streamp strm;
int check;
{
  struct inflate_state FAR* state;
...
unsigned long ZEXPORT inflateCodesUsed(strm) z_streamp strm;
{
  struct inflate_state FAR* state;
  if (inflateStateCheck(strm))
