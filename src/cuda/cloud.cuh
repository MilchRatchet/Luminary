...
LUM_DEVICE_FUNC float cloud_extinction(const vec3 origin, const vec3 ray, const CloudLayerType layer) {
 ...
 */
LUM_DEVICE_FUNC CloudRenderResult
...
LUM_DEVICE_FUNC float clouds_render(
  ...
  for (int i = 0; i < 3; i++) {
    ...
    if (device.iteration_type != TYPE_LIGHT && device.scene.sky.cloud.atmosphere_scattering) {
      color  = add_color(color, sky_trace_inscattering(origin, ray, result.hit_dist - prev_start, transmittance, seed));
