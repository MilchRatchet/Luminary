...
static LuminaryResult _device_free_buffers(Device* device) {
  ...
  __DEVICE_BUFFER_FREE(light_linked_lists);
...
LuminaryResult device_update_light_tree_data(Device* device, LightTree* tree) {
  ...
  __DEVICE_BUFFER_ALLOCATE(light_linked_lists, tree->linked_lists_size);
  ...
  __FAILURE_HANDLE(device_staging_manager_register(
    device->staging_manager, tree->linked_lists_data, (DEVICE void*) device->buffers.light_linked_lists, 0, tree->linked_lists_size));
