...
inline void _ui_renderer_render_rounded_box_corner(UIRendererRoundedBoxArgs* args, uint32_t col, uint32_t shape_col) {
  ...
  Color256 disk_left = color256_load(args->disk_mask + args->shape_row * args->shape_mask_ld + shape_col_offset);
  Color256 base_dst  = color256_load(args->dst + col_offset);
  Color256 base_src  = color256_load(args->src + col_offset);
  ...
  if (args->use_border) {
    Color256 circle_left = color256_load(args->circle_mask + args->shape_row * args->shape_mask_ld + shape_col_offset);
    base                 = color256_alpha_blend(border, base, circle_left, mask_low16, mask_high16, mask_add, mask_full_alpha);
  }

  color256_store(args->dst + col_offset, base);
}

inline void _ui_renderer_render_rounded_box_top_center(UIRendererRoundedBoxArgs* args, uint32_t col) {
  Color256 border     = color256_set_1(args->border_color);
  Color256 background = color256_set_1(args->background_color);
  ...
  if (args->use_border) {
    ...
    base = color256_load(args->src + col_offset);
...
inline void _ui_renderer_render_rounded_box_center(UIRendererRoundedBoxArgs* args, uint32_t col) {
  Color256 background = color256_set_1(args->background_color);
  ...
  color256_store(args->dst + col_offset, base_src);
}

inline void _ui_renderer_render_rounded_box_left_right(UIRendererRoundedBoxArgs* args, uint32_t col, bool is_left) {
  Color256 border          = color256_set_1(args->border_color);
  Color256 background      = color256_set_1(args->background_color);
  Color256 mask_low16      = color256_set_1(0x00FF00FF);
  Color256 mask_high16     = color256_set_1(0xFF00FF00);
  ...
  if (args->use_border) {
    Color256 border_mask = (is_left) ? color256_set(0, 0, 0, 0, 0, 0, 0, 0xFF) : color256_set(0xFF, 0, 0, 0, 0, 0, 0, 0);
...
inline void _ui_renderer_render_rounded_box_row_kind_1(UIRendererRoundedBoxArgs* args) {
  ...
  for (; col < args->shape_mask_half_cols; col++) {
    _ui_renderer_render_rounded_box_corner(args, col, shape_col);

    shape_col++;
  }

  for (; col < args->cols - args->shape_mask_half_cols; col++) {
    _ui_renderer_render_rounded_box_top_center(args, col);
  }
...
inline void _ui_renderer_render_rounded_box_row_kind_2(UIRendererRoundedBoxArgs* args) {
  ...
  for (; args->row < end_row && args->row < args->height_clip; args->row++) {
    ...
    for (; col < args->cols - args->shape_mask_half_cols; col++) {
      _ui_renderer_render_rounded_box_center(args, col);
    }
...
inline void _ui_renderer_render_rounded_box_row_kind_3(UIRendererRoundedBoxArgs* args) {
  for (; args->row < args->rows - args->shape_mask_half_size && args->row < args->height_clip; args->row++) {
    uint32_t col = 0;

    for (; col < 1; col++) {
      _ui_renderer_render_rounded_box_left_right(args, col, true);
    }
    ...
    for (; col < args->cols; col++) {
      _ui_renderer_render_rounded_box_left_right(args, col, false);
    }
...
static void _ui_renderer_render_rounded_box(
  ...
  _ui_renderer_render_rounded_box_row_kind_1(&args);
  _ui_renderer_render_rounded_box_row_kind_2(&args);
  _ui_renderer_render_rounded_box_row_kind_3(&args);
  _ui_renderer_render_rounded_box_row_kind_2(&args);
  _ui_renderer_render_rounded_box_row_kind_1(&args);
}
