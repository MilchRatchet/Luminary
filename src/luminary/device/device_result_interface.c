...
static LuminaryResult _device_result_interface_entry_allocate(DeviceResultInterface* interface, DeviceResultEntry* entry) {
  ...
  CUDA_FAILURE_HANDLE(cuEventCreate(&entry->download_event, CU_EVENT_DISABLE_TIMING));
  CUDA_FAILURE_HANDLE(cuEventCreate(&entry->upload_event, CU_EVENT_DISABLE_TIMING));
...
static LuminaryResult _device_result_interface_entry_free(DeviceResultInterface* interface, DeviceResultEntry* entry) {
  ...
  CUDA_FAILURE_HANDLE(cuEventDestroy(entry->download_event));
  CUDA_FAILURE_HANDLE(cuEventDestroy(entry->upload_event));
...
LuminaryResult device_result_interface_queue_result(DeviceResultInterface* interface, Device* device) {
  ...
  CUDA_FAILURE_HANDLE(cuStreamWaitEvent(device->stream_main, entry->upload_event, CU_EVENT_WAIT_DEFAULT));
  ...
  CUDA_FAILURE_HANDLE(cuEventRecord(entry->download_event, device->stream_main));
...
LuminaryResult device_result_interface_gather_results(DeviceResultInterface* interface, Device* device) {
  ...
  for (uint32_t result_id = 0; result_id < num_queued_results; result_id++) {
    ...
    CUDA_FAILURE_HANDLE(cuStreamWaitEvent(device->stream_main, entry->download_event, CU_EVENT_WAIT_DEFAULT));
    ...
    CUDA_FAILURE_HANDLE(cuEventRecord(entry->upload_event, device->stream_main));
