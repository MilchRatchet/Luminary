...
__device__ vec3 camera_sample_aperture(const ushort2 pixel_coords) {
...
#ifndef CAMERA_DEBUG_RENDER
  const float2 random = random_2D(RANDOM_TARGET_LENS, pixel_coords);
#else
  ...
  switch (device.camera.aperture_shape) {
    ...
    case LUMINARY_APERTURE_BLADED: {
      const int blade   = random_1D(RANDOM_TARGET_LENS_BLADE, pixel_coords) * device.camera.aperture_blade_count;
